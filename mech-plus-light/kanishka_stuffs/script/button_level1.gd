extends Area2D
@onready var animation_player_2: AnimationPlayer = $"../danger2/AnimationPlayer2"
...
@export var disappear_on_active_1 := false
@export var disappear_on_active_2 := false
@export var stop_on_active := false
...
func _show_new_object() -> void:
	...
	#if danger:
		#danger.visible = true
		#if danger.has_node("CollisionShape2D"):
			#danger.get_node("CollisionShape2D").disabled = false
			#
